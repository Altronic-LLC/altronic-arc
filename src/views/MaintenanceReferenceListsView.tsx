...
import { ArrowLeft, ArrowUpFromLine, Building2, Check, MapPin, Plus, X } from "lucide-react";
import {
...
// =============================================================================
// Maintenance → Departments & Locations — /operations/maintenance/reference-lists.
//
...
//
// **Inside the maintenance module, not under /admin.** Managing these lists is
// maintenance work done by maintenance people with the module already open,
// the same call as the asset register (`MaintenanceAssetsView`) — this screen
// used to live at /admin/maintenance-reference-lists behind the app-wide
// Admins gate, which meant a maintenance admin who wasn't also an ARC admin
// couldn't reach it (Ray, 2026-09-01: "locations and departments need moved
// from admin lists to the app and gated by the Maintenance roles list").
// Reading is open to anyone signed in — everybody has to be able to see what
// a value on the record in front of them means — and every WRITE asks
// `manageAssetsGate`, both here (to grey the controls) and again inside each
// `mutationFn`, exactly like the asset register.
//
...
export default function MaintenanceReferenceListsView() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-4 sm:px-6 sm:py-6">
      ...
      <div className="mb-6">
        <h1 className="font-display text-2xl font-semibold text-fg">Departments & Locations</h1>
      ...
      {!gate.allowed && !gate.resolving && (
        <p className="mb-5 rounded-md border border-ajax-yellow/40 bg-ajax-yellow/5 px-3 py-2 text-xs text-fg">
          {gate.hint} You can still see every department and location below.
        </p>
      )}
      ...
      <ReferenceListPanel key={tab.kind} tab={tab} canEdit={gate.allowed} editHint={gate.hint} />
    </div>
  );
}
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-2 sm:flex-row sm:items-end">
          <label className="flex flex-1 flex-col gap-1">
            ...
            <input
              ...
              className="rounded-md border border-border bg-bg px-3 py-2 text-base text-fg placeholder:text-fg-muted focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 sm:text-sm disabled:cursor-not-allowed disabled:opacity-50"
              disabled={create.isPending || !canEdit}
              title={canEdit ? undefined : editHint}
            />
          </label>
          <button
            type="submit"
            disabled={!newTitle.trim() || create.isPending || !canEdit}
            title={canEdit ? undefined : editHint}
    ...
    <div className="group flex items-center justify-between gap-2 rounded-md border border-border bg-surface px-3 py-2 transition-colors hover:border-fg-muted hover:bg-surface-2">
      ...
      <div className="flex shrink-0 items-center gap-2">
        <button
          ...
          disabled={!canEdit}
          title={canEdit ? undefined : editHint}
          className="rounded px-1.5 py-1 text-xs text-fg-muted opacity-0 transition-opacity hover:bg-surface hover:text-fg group-hover:opacity-100 focus:opacity-100 disabled:cursor-not-allowed disabled:opacity-0 group-hover:disabled:opacity-50"
          aria-label={`Rename ${value.title}`}
        >
          Rename
        </button>
        {value.active ? (
          <button
            ...
            disabled={!canEdit}
            className="inline-flex items-center gap-1 rounded px-1.5 py-1 text-xs text-fg-muted opacity-0 transition-opacity hover:bg-surface hover:text-fg group-hover:opacity-100 focus:opacity-100 disabled:cursor-not-allowed disabled:opacity-0 group-hover:disabled:opacity-50"
            // Not "Delete": records already pointing at it keep showing it,
            // and saying otherwise would promise something this can't do.
            title={
              canEdit
                ? "Take it out of the pickers. Records already using it keep showing it."
                : editHint
            }
          ...
          <button
            ...
            disabled={!canEdit}
            title={canEdit ? undefined : editHint}
            className="inline-flex items-center gap-1 rounded px-1.5 py-1 text-xs text-accent hover:underline disabled:cursor-not-allowed disabled:opacity-50"
