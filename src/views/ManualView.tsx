...
const SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    searchText:
      "Maintenance is the CMMS under Operations, backed by the Altronic Maintenance Tasks list on the Altronic_PMO SharePoint site. A work order is one job on one machine. The WO number is generated by ARC as WO-YYYY-#### and is read-only. Status is Backlog, Up Next, Started, Awaiting Parts, On Hold, Complete or Canceled - Awaiting Parts is its own board column so work blocked on supply is visible instead of being lumped in with On Hold. Priority is Low, Med, High or Emergency. Category is Corrective / Repair, Preventive, Inspection, Calibration, Cleaning, Oil Change, Safety or Improvement. Task Type is derived, not picked: Regular Maintenance when the work order came off a PM schedule, Request when somebody raised it directly. Due Status (On-Track / Late) is displayed but never edited in ARC - a Power Automate flow maintains that column. Only the assignee or an admin can mark a work order Complete; if nobody is assigned, whoever completes it is assigned in the same action so the record shows who did the job. The button is disabled with the reason on it rather than failing after the fact, and the same rule applies to dragging a card onto the board's Complete column. The list opens on the open queue and is deliberately not filtered to you - a maintenance backlog is a shared queue. Filters (equipment, assigned, category, department, search) live in the URL so a filtered view is shareable and survives the switch between list and board. The write-up fields - Failure Cause, Resolution, Parts Used, Tech Notes, labour hours and downtime hours - are edited behind one Edit button on the card. Comments, @-mentions, watchers and attachments work exactly as they do on a task.",
    render: () => (
      <>
        <P>
        ...
        <P>
          Only the <strong>assignee</strong> or an <strong>admin</strong>. For
          anyone else the Complete button is disabled with the reason on it and
          Complete is dropped from the status picker, rather than letting you
          try and fail — and the same rule applies to dragging a card onto the
          board's Complete column.
        </P>
        <P>
          If a work order is <strong>unassigned</strong>, anybody can complete
          it — and doing so <strong>assigns it to you</strong> in the same
          action, so the record shows who did the job. The page tells you that
          before you press it.
        </P>
