...
// EIRs, Test Sheets, Build Requests, …). Each wired type shows its count of
// ACTIVE items, a graphical status mini-bar, and clicks through to that type's
// page. Types whose SharePoint list isn't built yet render as disabled
// placeholders so the shape of the platform is visible before the data exists.
//
// As new department lists come online (Operations, Maintenance, etc.), swap a
// placeholder for a real card: add a hook, compute active count + segments,
// and point `to` at the new route.
// =============================================================================

// Bar-segment colours per status (stronger than the pill tints in atoms.ts so
// the stacked bar reads clearly). Complete/Closed are excluded from "active".
const TASK_BAR_COLOR: Record<Status, string> = {
  BACKLOG: "bg-fg-muted/40",
  "SELECTED FOR DEVELOPMENT": "bg-superior-blue",
  "In Progress": "bg-ajax-yellow",
  "On Hold": "bg-fg-muted/60",
...
const EIR_BAR_COLOR: Record<EirStatus, string> = {
  ...
  "Response Not Accepted": "bg-cooper-red/60",
  Closed: "bg-fg-muted/60",
};
...
export function DashboardView() {
  ...
  const taskCard = useMemo(() => {
    const active = tasks.filter((t: Task) => t.status !== "Complete");
  ...
  const eirCard = useMemo(() => {
    const active = eirs.filter((e: Eir) => e.status !== "Closed");
    ...
    <div className="mx-auto flex max-w-[1200px] flex-col gap-5 px-4 py-4 sm:px-6 sm:py-6">
      <header>
        ...
        <p className="mt-1 text-sm text-fg-muted">
          {greet(currentUser.displayName)} — here's the active work across ARC.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <TypeCard
          ...
          onClick={() => navigate("/list?assigned=")}
        />
        <TypeCard
          name="EIRs"
