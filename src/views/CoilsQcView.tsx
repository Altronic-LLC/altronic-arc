...
export function CoilsQcView() {
  const { data: entries = [], isLoading, error } = useCoilDefectLog();
  const { data: partNumbers = [] } = useCoilPartNumbers();
  const { data: otherFaults = [] } = useCoilOtherFaults();
  ...
  return (
    <div className="mx-auto flex max-w-[1200px] flex-col gap-5 px-4 pb-1 pt-4 sm:px-6 sm:pb-2 sm:pt-6">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <button type="button" onClick={() => setEditingEntry(null)} className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-2 text-sm font-medium text-white hover:bg-accent/90"><Plus className="h-4 w-4" />Add entry</button>
      </header>
      ...
      {isLoading ? <LoadingTasks noun="the coil defect log" /> : error ? (
