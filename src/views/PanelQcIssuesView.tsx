...
export function PanelQcIssuesView() {
  ...
  const [editing, setEditing] = useState<PanelQcIssue | null>(null);
  ...
  return <div className="mx-auto flex max-w-[1500px] flex-col gap-4 px-4 py-4 sm:px-6 sm:py-6">
    <header className="flex flex-wrap items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg bg-cooper-red/10 text-cooper-red"><ClipboardCheck className="h-5 w-5" /></span><div className="min-w-0 flex-1"><h1 className="font-display text-xl font-semibold text-fg sm:text-2xl">Panel QC Issue Tracker</h1><p className="text-sm text-fg-muted">Track panel and board defects from production through resolution.</p></div><button onClick={() => setNewIssue(true)} className="inline-flex items-center gap-1.5 rounded-md bg-accent px-3 py-1.5 text-sm font-medium text-white hover:bg-accent/90"><Plus className="h-4 w-4" />New Issue</button></header>
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-surface"><div className="border-b border-border bg-surface-2 px-4 py-2.5 text-sm font-medium text-fg">{isLoading ? "Loading…" : `${filtered.length} issue${filtered.length === 1 ? "" : "s"}`}{query && !isLoading && <span className="ml-1 text-fg-muted">of {issues.length}</span>}</div>
      ...
      {isLoading ? <LoadingTasks noun="Panel QC issues" /> : filtered.length === 0 ? <div className="px-4 py-10 text-center text-sm text-fg-muted">{query ? "No issues match that search." : "No issues yet. Click New Issue to record the first."}</div> : <><div className="divide-y divide-border sm:hidden">{visible.map((issue) => <IssueCard key={issue.id} issue={issue} onEdit={() => setEditing(issue)} />)}</div><div className="hidden overflow-x-auto sm:block"><table className="w-full table-fixed text-left text-sm"><thead className="bg-surface-2 text-[11px] uppercase tracking-wider text-fg-muted"><tr>{SORT_COLUMNS.map(({ key, label }) => <th key={key} aria-sort={sortKey === key ? sortDirection === "asc" ? "ascending" : "descending" : "none"} className="whitespace-nowrap px-2 py-2 font-semibold"><button type="button" onClick={() => toggleSort(key)} className="inline-flex items-center gap-1 hover:text-fg">{label}{sortIcon(key)}</button></th>)}<th className="w-9 px-1 py-2"><span className="sr-only">Edit</span></th></tr></thead><tbody>{visible.map((issue) => <IssueRow key={issue.id} issue={issue} onEdit={() => setEditing(issue)} />)}</tbody></table></div></>}
    </div>
    {newIssue && <PanelQcIssueFormModal onClose={() => setNewIssue(false)} />}{editing && <PanelQcIssueFormModal issue={editing} onClose={() => setEditing(null)} />}
  </div>;
}

function IssueRow({ issue, onEdit }: { issue: PanelQcIssue; onEdit: () => void }) { return <tr onClick={onEdit} className="cursor-pointer border-t border-border align-top hover:bg-surface-2"><td className="break-words px-2 py-2 font-medium text-fg">{display(issue.panelSerialNumber)}</td><td className="break-words px-2 py-2 text-fg-muted">{formatSpDate(issue.date)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.partNumber)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.partDescription)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.serialReferenceNote)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.defectCategory)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.comments)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.correctiveAction)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.productionTechnician)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.productionRepairNotes)}</td><td className="break-words px-2 py-2 text-fg-muted">{display(issue.productionResolution)}</td><td className="w-9 px-1 py-2"><button type="button" onClick={(event) => { event.stopPropagation(); onEdit(); }} aria-label={`Edit issue ${issue.panelSerialNumber}`} className="rounded-md p-1 text-fg-muted hover:bg-surface-2 hover:text-fg"><Pencil className="h-4 w-4" /></button></td></tr>; }

function IssueCard({ issue, onEdit }: { issue: PanelQcIssue; onEdit: () => void }) { return <button type="button" onClick={onEdit} className="flex w-full flex-col gap-2 px-4 py-3 text-left hover:bg-surface-2"><div className="flex items-start justify-between gap-2"><span className="font-medium text-fg">{display(issue.panelSerialNumber)}</span><Pencil className="h-4 w-4 shrink-0 text-fg-muted" /></div><dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-sm"><dt className="text-fg-muted">Date</dt><dd className="truncate text-right text-fg">{formatSpDate(issue.date)}</dd><dt className="text-fg-muted">Part</dt><dd className="truncate text-right text-fg">{display(issue.partNumber)}</dd><dt className="text-fg-muted">Defect</dt><dd className="truncate text-right text-fg">{display(issue.defectCategory)}</dd><dt className="text-fg-muted">Description</dt><dd className="truncate text-right text-fg">{display(issue.partDescription)}</dd></dl></button>; }