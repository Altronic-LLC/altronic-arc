      ...
      <form onSubmit={submit} className="min-h-screen w-full bg-surface sm:min-h-0 sm:max-w-5xl sm:rounded-lg sm:border sm:border-border sm:shadow-xl">
        ...
        <div className="grid gap-5 px-4 py-5 sm:p-5">
          ...
          {showAddPart && <div className="flex flex-wrap items-end gap-2 border-b border-border pb-4">
            <label className="flex min-w-56 flex-1 flex-col gap-1 text-sm text-fg-muted">Add coil part number<input value={newPartNumber} onChange={(event) => setNewPartNumber(event.target.value)} placeholder="New part number" className="rounded-md border border-border bg-surface-2 px-3 py-2 text-base text-fg sm:text-sm" /></label>
          ...
          <section>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {COIL_DEFECT_FIELDS.map((field) => <label key={field} className="flex flex-col gap-1 text-sm text-fg-muted">{labelFor(field)}<input type="number" min="0" value={draft.defects[field]} onChange={(event) => setDefect(field, event.target.value)} className="rounded-md border border-border bg-surface-2 px-3 py-2 text-base text-fg sm:text-sm" /></label>)}
              <label className="flex flex-col gap-1 text-sm text-fg-muted">Failed<input type="number" min="0" value={draft.failed} onChange={(event) => setDraft((current) => ({ ...current, failed: Math.max(0, Number(event.target.value) || 0) }))} className={`rounded-md border bg-surface-2 px-3 py-2 text-base sm:text-sm ${failedMismatch ? "border-cooper-red" : "border-border"} text-fg`} /></label>
            </div>
